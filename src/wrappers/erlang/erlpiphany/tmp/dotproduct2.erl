...
spinwait(_, _, 0) ->
    io:format("spinwait timeout~n", []),
    error;
spinwait(0, Dev, T) ->
    timer:sleep(1),
    {ok, {4, Done}} = ehal:e_read(Dev, 0, 0, 16#7000, 4),
    [B1, B2, B3, B4] = binary_to_list(Done),
    spinwait(B1+B2+B3+B4, Dev, T-1);
spinwait(X, _, T) when X/=0, T>0 ->
    io:format("spinwait Done=~p~n", [X]),
    ok.
